...
Data types are used to represent different types of data in R. R has several built-in data types, such as `numeric`, `character`, `integer`, `complex`, and `logical`. Data types are used to store different types of data, such as numbers, text, and logical values. Below are examples for your learning.

```{r}
x <- "Hello Charles, I hope you're working hard to become who you were destined to be by God's purpose?"
x
...
```{r}
kx <- data.frame(age = c(11,14,22), weight = c(30,33,32)) # creating a dataframe kx
kx
```

```{r}
d <- c("My name is Charles Kwame Appiah") # creating a vector
d
class(d) # checking the class of variable d
length(d) # checking the length of variable d
nchar(d) # counting the number of characters in variable d
```

```{r}
f<- c(1,3,4,6,7) # creating a vector f
f
...
Matrix is a two-dimensional array. It is created with the `c()` function. The `c()` function takes a vector as input and reshapes it into a matrix . The `nrow` argument specifies the number of rows in the , and the `ncol` argument specifies the number of columns in the .

```{r}
mat <- c(2,4,5,7) # creating a vector
dim(mat) <- c(2,2) # creating a  with 2 rows and 2 columns
mat
solve(mat) # Solving the matrix 

mar = c(2,4,5,7,8,9,0,1,3) # creating a vector
dim(mar) <- c(3,3) # creating a  with 3 rows and 2 columns
mar
solve(mar) # Solving the matrix
```
...
```{r}
# creating a  with 3 rows and 3 columns
wr <- matrix(1:9, nrow = 3, ncol = 3, by = TRUE) 

wr # creating a  with 3 rows and 3 columns
...
```{r}
(d <- factor (c("Yes", "No","Yes", "No"), levels = c("Yes", "No"), ordered = FALSE))
```
...
# Second Row
dfr[2,] # accessing row 2 of the dataframe

# First Column
dfr[,1] # accessing column 1 [Username] of the dataframe

# Second Column
dfr[,2] # accessing column 1 of the dataframe
```
...
# Select only Username Column
dfr$Username

# Select only Adult Column
dfr$Adult
```
...
woo$age # select only the age list

woo$height # select only the height list
...
# tail(dat) # displaying the last few rows of the dataset
...
tail(data) # displaying the last few rows of the dataset
...
Sequence is a collection of numbers in a specific order. Sequences can be created in R using the semicolon`:` operator, the `seq()` function, and the `rep()` function. The `:` operator is used to create a sequence of numbers from a starting value to an ending value. The `seq()` function is used to create a sequence of numbers with a specified start, end, and step size. The `rep()` function is used to repeat a sequence of numbers for a specified number of times.
...
if (x < 0) {
print("Negative!") # check if x is negative
}else if (x < 10) {
print("Positive, less than 10!") # check if x is less than 10
}else {
print("Number is greater than 10!, Correct") # check if x is greater than 10
}
```
...
x <- -20 # asign -20 to x 
if (x < 0) {
print("Negative!, Correct") # check if x is negative
}else if (x < 10) {
print("Positive, less than 10!") # check if x is less than 10
}else {
print("Number is greater than 10!") # check if x is greater than 10
}
```
...
print("Positive, less than 10!, Correct") # check if x is less than 10
}else {
print("Number is greater than 10!") # check if x is greater than 10
}
```
...
print(x)
if (x > 7){
break    # break when x is greater than 7
}
x <- x + 1
}
```
...
print(x)
if (x <= 9){
break # break when x is less than 9
}
x <- x + 1    
}
```
...
`sapply` is used to apply a function to each element of a list and simplify the result. sapply returns a vector or instead of a list.
...
adult = c(FALSE,TRUE,FALSE,TRUE,FALSE,TRUE))

(s <- split(dat, dat$adult)) # splitting based on the adult column 

# split them according to True and False
```
...
`tapply` is used to apply a function to each group of a vector. tapply returns a vector or , where each element is the result of applying the function to the corresponding group of the input vector.
...
class(airquality) # check the class of airquality

airquality # display airquality dataset
...
filter(airquality, Month > 5 | Month < 9, Day < 3) # filter values in Month greater than 5 or less than 9, Day values less than 3

filter(airquality, Day == 1 | Day == 5) # filter values of Day = 1 or 2
...
# Finding the minimum, mean and maximum value by Ozone
summarize (by_month, min(Ozone), mean(Ozone), max(Ozone))

# Finding the minimum, mean and maximum value by Month
summarize (by_month, min(Day), mean(Day), max(Day))

# Finding the minimum, mean and maximum value by Wind
...
airquality |> transmute(Month_Day = Month + Day, Temp_Wind = Temp + Wind)

airquality |> transmute(Month_Day = Month + Day, Temp_Wind = Temp + Wind) |> ggplot() + geom_point(aes(x = Month_Day, y = Temp_Wind), col = "blue")

airquality |> transmute(Mean_wind = mean(Wind), Mean_Temp = mean(Temp)) |> ggplot() + geom_point(aes(x = Mean_wind, y = Mean_Temp))
```
...
`Statistical summary` and `visualization` are important steps in data analysis. Statistical summary provides a summary of the data, such as the mean, median, and standard deviation. Visualization provides a visual representation of the data, such as bar charts and histograms etc.
...
which.min(A) # index of maximum number

which.max(A) # index of minimum number
```
...
summary(A)
fivenum(A) # Omit the mean of the variable
```
...
# sum((course - mean(course)) ^ 2 / (length(course) - 1)) # Same as variance

sd(course)

# sqrt(var(course)) # Same as sd
...
```{r}
sqrt(course) # square root of course

sum(course) # sum of course

prod(course) # product of course

sort(course) # sort values of course

order(course) # order values of course based on idexes

sqrt(-14 + 9i) # square root of complex number
```
...
prop.table(table(mo)) # creating a table of mo based on proportion
...
```{r}
barplot(table(mo), col = c("red", "blue", "black","green" )) # creating a bar chart of mo

barplot(prop.table(table(mo)), col = c("orange", "pink", "purple","violet" )) # creating a bar chart of mo
```
...
`A contingency` is used to visualize the relationship between two categorical variables. A contingency is a two-dimensional table that shows the frequency of each combination of categories in the two variables. A contingency is used to visualize the relationship between two categorical variables.

```{r}
g = c(rep("Male",8), rep("Female",12)) # creating a vector
g

mg = table(mo,g) # creating a contingency  of mo and g
mg
...
```{r}
barplot(mg, col = c("orange","blue", "pink","lightgreen")) # creating a bar chart of mg

barplot(mg,xlim=c(0,2), xlab ="Gender", length =levels(mo), col = c("orange","black", "pink","green")) # creating a bar chart of mg

barplot(mg,xlim=c(0,2), xlab = "Transportation", length =levels(mo), col = c("orange","blue", "pink","green")) # creating a bar chart of mg

```

```{r}
barplot(prop.table(mg,1), width=0.25, xlim=c(0,3), ylim=c(0,1), xlab="Gender", legend=levels(mo), beside=T, col = c("orange","green")) # create a bar chart of mg

mg = table(g,mo) # creating a contingency  of g and mo
barplot(prop.table(mg,2), width=0.25, xlim=c(0,3), ylim=c(0,1), xlab="Transportation", legend=levels(g), beside=T, col= c("pink", "purple")) # create a bar chart of mg
...
```{r}
xo = c(10, 10, 5, 9, 7, 6,8,6,5,8, 10, 7, 7,8, 5, 6,4,7,9,7, 4,8, 10,10, 7,4,9,5,8,9) # creating a vector of xo
...
```{r}
hist(xo, col = c("orange","lightblue", "pink","lightgreen", "purple", "violet","brown")) # create a histogram of xo

hist(xo, nclass = 3, col = c("lightblue", "pink","lightgreen")) # create a histogram of xo

hist(xo, nclass = 5, col = c("lightblue", "lightgreen", "purple", "pink", "orange"))
```
...
hist(wo, col = c("orange","black", "pink","lightgreen")) # creating a histogram of wo

hist(wo, nclass=4, col = c("orange","black", "pink","lightgreen")) # creating a histogram of wo
```

```{r}
hist(wo, breaks= seq(from = 1000, to=5000, by=300), col = c("lightblue", "pink","lightgreen")) # creating a histogram of wo

hist(wo, probability=T, col = c("lightblue", "pink","lightgreen")) # creating a histogram of wo
...
```{r}
temp = hist(wo, col = c("pink","lightblue", "purple","lightgreen")) # creating a histogram of wo
temp
...
```{r}
boxplot(wo, horizontal = T, col = c("orange")) # create a boxplot of wo

boxplot(wo, vertical = T, col = c("lightblue")) # create a boxplot of wo
...
```{r}
w1 = c(1950, 2090, 2700, 3350, 4200, 3720, 4400, 2980, 3850, 4550, 3050, 2350,1850, 2820, 3670, 2950, 3750, 1850, 2420,3150, 3000, 3470, 3920, 3100, 2400, 2900, 2650, 3450, 3650, 4020) # create a vector of w1
...
boxplot(w1, w2, names = c("Sample 1", "Sample 2"), col = c("purple", "pink" )) # creating a boxplot of w1 and w2
```

x <- "Hello Charles, I hope you're working hard to become who you are destined to be? "
x
...
is.na(x) # checking if number is not available 
...
x <- list(a=rnorm(10), b=rnorm(20), c=rnorm(30))
sapply(x, mean) # checking the mean of x
sapply(x, var) # checking the variance of x
sapply(x, sd) # checking the standard deviation of x
...
barplot(mg,xlim=c(0,2), xlab="Transportation", length=levels(g), col= 1:2)

barplot(prop.table(mg,1), width=0.25, xlim=c(0,3), ylim=c(0,1), xlab="Gender", legend=levels(mo), beside=T, col= 1:4)

mg = table(g,mo)
barplot(prop.table(mg,2), width=0.25, xlim=c(0,3), ylim=c(0,1), xlab="Transportation", legend=levels(g), beside=T, col= 1:2)
...
hist(wo)
hist(wo, nclass=4)
hist(wo, breaks= seq(from = 1000, to=5000, by=300))
hist(wo, probability=T)
rug(jitter(wo))
...
boxplot(wo, horizontal = T)
boxplot(wo, vertical = T)
fivenum(wo)
summary(wo)

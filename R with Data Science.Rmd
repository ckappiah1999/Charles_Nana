...
is a two-dimensional array. It is created with the `()` function. The `()` function takes a vector as input and reshapes it into a . The `nrow` argument specifies the number of rows in the , and the `ncol` argument specifies the number of columns in the .
...
mati <- (temp, nrow = 2, ncol = 3,byrow = TRUE) # creating a  with 2 rows and 3 columns
mati

temp <- c(3,4,5,5.6,6,7, 8, 9, 10) # creating a vector
mato <- (temp, nrow = 3, ncol = 3,byrow = TRUE) # creating a  with 3 rows and 3 columns
mato
```
...
# Default byrow = FALSE
temp <- c(3,4,5,5.6,6,7) # creating a vector
mati <- (temp, nrow = 2, ncol = 3,byrow = FALSE) # creating a  with 2 rows and 3 columns
mati
```
...
airquality %>% transmute(Month_Day = Month + Day, Temp_Wind = Temp + Wind) %>% ggplot() + geom_point(aes(x = Month_Day, y = Temp_Wind))

airquality %>% transmute(Mean_wind = mean(Wind), Mean_Temp = mean(Temp)) %>% ggplot(aes(x = Mean_wind, y = Mean_Temp)) + geom_line()
```

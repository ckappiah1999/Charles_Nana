...
iris %>% tbl_df %>%
select(Sepal.Length:Petal.Length) %>% head() ## Select from Sepal.Width to Petal.length

iris %>% tbl_df %>%
select(starts_with("Petal")) %>% head() ## Select Petal.Width and Length only
...
```{r warning=FALSE}
iris %>% tbl_df %>%
 mutate(Petal.Width.plus.Length = Petal.Width + Petal.Length) %>%select(Species, Petal.Width.plus.Length) %>%
 head()
```
```{r warning=FALSE}
iris %>% tbl_df %>%
 mutate(Sepal.Width.plus.Length = Sepal.Width + Sepal.Length) %>%select(Species, Sepal.Width.plus.Length) %>%
 head()
```
```{r warning=FALSE}
...
iris %>% tbl_df %>%
 ...
 head() ### from smallest to largest in Sepal.Length column
```
```{r warning=FALSE}
iris %>% tbl_df %>%
...
cars %>% lm(dist ~ speed, data = .) %>% abline(col = "green")
```
```{r}
...
"virginica" = "Virginica")) +
 ...
 facet_grid(Measurement ~ ., switch = "y",
 labeller = labeller(Measurement = label_map)) + coord_flip()
```
```{r}
iris %>% gather(Measurement, Value, -Species) %>%
 ggplot(aes(x = Species, y = Value, fill = Species)) +
...
mapped_votes %>%as.data.frame %>% cbind(Class = HouseVotes84$Class) %>%
...
class(x)
is.character(x)
as.character(3.14) # converting it to character data type
rep("foo", 10)
v <- 1:3
is.atomic(v)
v <- 1:3

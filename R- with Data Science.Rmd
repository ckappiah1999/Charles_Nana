...
### **Controls loops** {#sec-controls-loops}
...
```{r}
iris %>% tbl_df %>%
select(Sepal.Width, Petal.Width, Species) %>% head() ## Select Sepal.Width, Petal.Width and Species from iris
```

```{r Excluding Petal.Width }
# Select from Sepal.Width to Petal.length
iris %>% tbl_df %>% select(Sepal.Length:Petal.Length) %>% head() 
```

```{r Petal only}
# Select Petal.Length and Width only
iris %>% tbl_df %>% select(starts_with("Petal")) %>% head() 
```

```{r Length Only, warning=FALSE}
# Select Petal.Length and Sepal.Length only
iris %>% tbl_df %>% select(ends_with("Length")) %>% head()
```

```{r etal Only}
# Select columns containing etal only
iris %>% tbl_df %>% select(contains("etal")) %>% head()
```
...
iris %>% tbl_df %>% select(matches(".t.")) %>% head()
...
iris %>% tbl_df %>% mutate(Petal.Width.plus.Length = Petal.Width + Petal.Length) %>%select(Species, Petal.Width.plus.Length) %>% head()
```
...
iris %>% gather(key = Attribute, value = Measurement,
 Petal.Length, Petal.Width) %>% select(Species, Attribute, Measurement) %>% head() 
```
...
cars %>% lm(dist ~ speed, data = .) %>% abline(col = "red") # fitting a line to the plot 
```
...
```{r Cars}
cars %>% ggplot + geom_histogram(aes(x = speed), bins = 10, col = "blue")

cars %>% ggplot + geom_density(aes(x = speed))

cars %>% ggplot + geom_histogram(aes(x=dist), bins = 10, col = "pink")
...
iris %>% gather(Measurement, Value, -Species) %>% ggplot(aes(x = Species, y = Value)) + geom_boxplot() + facet_grid(Measurement ~ ., scale = "free_y", labeller = labeller(Measurement = label_map))
```
...
longley %>% gather(key, value, Unemployed, Armed.Forces) %>% ggplot(aes(x = Year, y = value)) + geom_line() + geom_point() +scale_x_continuous(breaks = 1947:1962) + facet_grid(key ~ .)
```
...
```{r Color.Species}
iris %>% gather(Measurement, Value, -Species) %>% ggplot(aes(x = Species, y = Value, fill = Species)) + geom_boxplot() + facet_grid(Measurement ~ ., scale = "free_y", labeller = labeller(Measurement = label_map))

iris %>% gather(Measurement, Value, -Species) %>% ggplot(aes(x = Species, y = Value, fill = Species)) + geom_boxplot() + scale_fill_manual(values = c("red", "green", "blue")) + facet_grid(Measurement ~ ., scale = "free_y", labeller = labeller(Measurement = label_map)) 
```
